...
const fontSizes = (() => {
  ...
  const s = font?.size as Record<string, { $value: number }> | undefined;
  if (!s) return [];
  return Object.entries(s)
    .map(([key, v]) => {
      const px = v.$value;
      const rem = (px / 16).toFixed(3);
      const baseUnit = (px / 4).toFixed(1);
      return {
        token: `font.size.${key}`,
        rem,
        baseUnit,
...
export const FontSizeDoc: Story = {
  render: () => {
    ...
    fontSizes.forEach(({ token, rem, baseUnit, px, cssVar }) => {
      ...
      tr.innerHTML = `
        ...
        <td style="padding: ${cellPad}; border-bottom: ${cellBorder};">
          <span style="font-size: ${px}px; line-height: 1.2;">Aa</span>
        </td>
      `;
