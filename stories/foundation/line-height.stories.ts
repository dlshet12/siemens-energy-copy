...
const lineHeights = (() => {
  ...
  const line = font?.line as Record<string, { $value: number }> | undefined;
  if (!line) return [];
  const entries = Object.entries(line).map(([key, v]) => {
    const px = v.$value;
    const rem = (px / 16).toFixed(3);
    const baseUnit = (px / 2).toFixed(1);
    return {
      token: `line-height.${key === '00' ? '10' : key}`,
      rem,
      baseUnit,
      px,
      cssVar: `--font-line-${key}`
    };
  });
  return entries.sort((a, b) => a.px - b.px);
})();
...
export const LineHeightDoc: Story = {
  render: () => {
    ...
    table.innerHTML = `
      <thead>
        <tr>
          ...
          <th style="text-align:right; padding: ${cellPad}; border-bottom: ${cellBorder}; font-weight: 600;">px</th>
    ...
    lineHeights.forEach(({ token, rem, baseUnit, px, cssVar }) => {
      ...
      tr.innerHTML = `
        ...
        <td style="text-align:right; padding: ${cellPad}; border-bottom: ${cellBorder};">${rem}</td>
        <td style="text-align:right; padding: ${cellPad}; border-bottom: ${cellBorder};">${baseUnit}</td>
        <td style="text-align:right; padding: ${cellPad}; border-bottom: ${cellBorder};">${px}</td>
        <td style="padding: ${cellPad}; border-bottom: ${cellBorder};">
          <div style="font-size: 12px; line-height: ${px}px; max-width: 80px;">Sample line text</div>
        </td>
      `;
