...
const fontWeights = (() => {
  ...
  const w = font?.weight as Record<string, { $value: number }> | undefined;
  if (!w) return [];
  const order = ['light', 'regular', 'medium', 'semi-bold', 'bold'];
  return order
    .filter((k) => w[k])
    .map((k) => ({
      token: `font.weight.${k}`,
      value: (w[k] as { $value: number }).$value,
...
export const FontWeightDoc: Story = {
  render: () => {
    ...
    fontWeights.forEach(({ token, value, cssVar }) => {
      ...
      tr.innerHTML = `
        ...
        <td style="padding: ${cellPad}; border-bottom: ${cellBorder};">
          <span style="font-weight: ${value}; font-size: 16px;">Sample text</span>
        </td>
      `;
